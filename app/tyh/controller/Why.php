    ...
    public function record()
    {
        ...
        if (request()->isAjax()) {
            ...
            if ($name == 'why') {
                ...
                $list = $list->where('r.responsible_person', 'why')->select();
...
//
//            $check_id=$data['id'];
//            $res=Db::name('healthcheck')->where('id',$check_id)->find();
//            if($res==null){
//                Db::name('healthcheck')->strict(false)->insert($data);
//                return to_assign();
//            }
//            else{
//                return to_assign(1,'检查id已存在');
//            }
//
//        } else {
//            $list=Db::name('healthcheck')->select();
